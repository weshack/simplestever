...
exports.authenticate = function(req, res) {
	...
	var client_info = ($.parseJSON(fs.readFileSync('cred/client.json').toString()))
	...
	client.authenticate(function(error, client) {
		...
		var userInfo = ($.parseJSON(fs.readFileSync('cred/users.json').toString())).users
		console.log(userInfo)
		// If user exists, get user, reset auth token, set/check cookie 
		...
		// else if user does not exist, create new user, add to users, set cookie 
		if newCheck	== true {
			console.log("user does not exist")
			userInfo.uid = {"uid":uid,"token":token}
		}
